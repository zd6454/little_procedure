...
const DB = wx.cloud.database().collection("system_strategy");
...
Page({
  ...
  onLoad: function (options) {
    ...
    DB.get({
      success: res => {
        ...
        this.data.system_strategy.forEach(v => {
        ...
        //   })
        // })
        console.log("查询成功", this.data.system_strategy);
        const that = this;
        wx.getUserInfo({
  ...
  handleIlike(e) {
    console.log(e);
    const system_strategy = this.data.system_strategy;
    ...
    console.log(this.data.system_strategy)
    wx.setStorageSync("system_strategy", this.data.system_strategy);
  ...
  //         if (res.userInfo.nickName === v.user || res.userInfo.nickName === that.data.system_strategy.userInfo.nickName){//评论的人和题主可以删评论
  //           wx.showModal({
  //             title: '提示',
  //             content: '是否确定删除',
  //             success(t) {
